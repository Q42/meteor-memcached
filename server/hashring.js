...
var
  ...
  /*hashValue = Npm.require('bindings')('hashvalue.node')
  ,*/ SimpleCache = SimpleLRUCache
  , parse = ConnectionParse
  ...
  servers.forEach(function each(server) {
    ...
    for (var i = 0; i < length; i++) {
      ...
      for (var j = 0; j < self.replicas; j++) {
        ...
        key = "server-" + i + ", repl-" + j; //hashValue.hash(x[3 + j * 4], x[2 + j * 4], x[1 + j * 4], x[j * 4]);
